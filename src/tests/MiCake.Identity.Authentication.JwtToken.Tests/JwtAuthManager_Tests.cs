...
using MiCake.Identity.Authentication.JwtToken.Tests.FakeUser;
...
namespace MiCake.Identity.Authentication.JwtToken.Tests
{
    public class JwtAuthManager_Tests
    {
        ...
        private readonly SymmetricSecurityKey Key = new(Encoding.Default.GetBytes("4045da7b-0462-4b81-8d10-07004e96d1ee"));
        ...
        {
            var supporter = CreateJwtAuthManager(s => { s.IssuerSigningKey = Key; });
        ...
        {
            var supporter = CreateJwtAuthManager(s => { s.UseRefreshToken = false; s.IssuerSigningKey = Key; });
        ...
        public async void DecodeJwtToken_UseWrongSecurityKey()
        {
            ...
            await Assert.ThrowsAsync<SecurityTokenSignatureKeyNotFoundException>(async () =>
            {
                ...
                {
                    s.IssuerSigningKey = new SymmetricSecurityKey(Encoding.Default.GetBytes("wrong-key"));
